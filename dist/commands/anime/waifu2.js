...
class NekosV4Cache {
  constructor({ cacheSize = 100, nsfw = false, tags = 'girl', exclude = 'boy,wet,dick' } = {}) {
    this.cache = [];
  ...
  async getRandom() {
    ...
    if (this.cache.length <= 20 &&!this.isFetching) {
      this.fillCache();
    }
...
const nekos = new NekosV4Cache({ cacheSize: 100, nsfw: false });
